...
import {
  constructMinerLabel, isTaskRunning,
...
export const useRulesMining = createSharedComposable(() => {
  ...
  const taskStateQuery = useTaskStateQuery(startedTaskId, {
    refetchInterval: (task, query) => {
      if (!task || query.state.status === 'loading') return false;
  ...
  const isLoading = computed(() => (
    createTaskMutation.isLoading.value ||
    startTaskMutation.isLoading.value ||
    taskStateQuery.isInitialLoading.value ||
    taskRulesQuery.isInitialLoading.value
  ));
